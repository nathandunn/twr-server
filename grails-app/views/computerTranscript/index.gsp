...
<html>
	...
	<body>
		<a href="#list-computerTranscript" class="skip" tabindex="-1"><g:message code="default.link.skip.label" default="Skip to content&hellip;"/></a>
		...
		<div id="list-computerTranscript" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="returnDate" title="${message(code: 'computerTranscript.returnDate.label', default: 'Return Date')}" />
					
						<g:sortableColumn property="transcript" title="${message(code: 'computerTranscript.transcript.label', default: 'Transcript')}" />
					
						<g:sortableColumn property="status" title="${message(code: 'computerTranscript.status.label', default: 'Status')}" />
				...
				<g:each in="${computerTranscriptInstanceList}" status="i" var="computerTranscriptInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${computerTranscriptInstance.id}">${fieldValue(bean: computerTranscriptInstance, field: "returnDate")}</g:link></td>
					
						<td>${fieldValue(bean: computerTranscriptInstance, field: "transcript")}</td>
