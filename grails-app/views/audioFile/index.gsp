			...
			<thead>
				...
				<g:each in="${audioFileInstanceList}" status="i" var="audioFileInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${audioFileInstance.id}">${fieldValue(bean: audioFileInstance, field: "audioData")}</g:link></td>
						...
						<td>${fieldValue(bean: audioFileInstance, field: "fileName")}</td>
					
						<td>${fieldValue(bean: audioFileInstance, field: "passage")}</td>
