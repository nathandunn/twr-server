...
<div class="fieldcontain ${hasErrors(bean: audioFileInstance, field: 'callbackUrl', 'error')} ">
	...
	<g:textField name="callbackUrl" value="${audioFileInstance?.callbackUrl}"/>
...
<div class="fieldcontain ${hasErrors(bean: audioFileInstance, field: 'passage', 'error')} required">
	...
	<g:select id="passage" name="passage.id" from="${edu.uoregon.secondlook.Passage.list()}" optionKey="id" required="" value="${audioFileInstance?.passage?.id}" class="many-to-one"/>
