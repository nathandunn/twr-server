...
class ResearcherDbRealm {
    ...
    def authenticate(authToken) {
        ...
        // exception.
        def user = ResearcherUser.findByUsername(username)
        if (!user) {
        ...
        // in the database.
        def account = new SimpleAccount(username, user.passwordHash, "ResearcherDbRealm")
    ...
    def hasRole(principal, roleName) {
        def roles = ResearcherUser.withCriteria {
            roles {
                eq("name", roleName)
            }
    ...
    def hasAllRoles(principal, roles) {
        def r = ResearcherUser.withCriteria {
            roles {
                'in'("name", roles)
            }
            eq("username", principal)
    ...
    def isPermitted(principal, requiredPermission) {
        ...
        def user = ResearcherUser.findByUsername(principal)
        ...
        def results = ResearcherUser.executeQuery("select distinct p from ResearcherUser as user join user.roles as role join role.permissions as p where user.username = '$principal'")
